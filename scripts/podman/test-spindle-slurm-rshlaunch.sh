#!/bin/bash
#
# Run Spindle Slurm rshlaunch tests in podman
#
# This runs the Spindle testsuite in a Slurm cluster with rshlaunch launcher.
...
NETWORK_NAME="slurm-rshlaunch-test-net"
WORKERS=4
...
echo "=========================================="
echo "Spindle Slurm Rshlaunch Tests"
echo "=========================================="
echo ""
...
cleanup() {
    ...
    for container in slurm-rshlaunch-mariadb slurm-rshlaunch-db slurm-rshlaunch-head slurm-rshlaunch-node-{1..4}; do
...
# DISABLED for debugging - cleanup manually with: podman rm -f slurm-rshlaunch-{mariadb,db,head,node-{1..4}}; podman network rm -f slurm-rshlaunch-test-net
# trap cleanup EXIT
...
MARIADB_PASSWORD=$(grep MARIADB_PASSWORD "$REPO_ROOT/containers/spindle-slurm-ubuntu/testing/mariadb.env" | cut -d'"' -f2)
if [ -z "$MARIADB_PASSWORD" ]; then
    echo "Error: Could not read password from mariadb.env"
    exit 1
fi
podman run \
    --name slurm-rshlaunch-mariadb \
...
podman exec slurm-rshlaunch-head bash -c 'munge -n | unmunge'
...
podman exec slurm-rshlaunch-head sinfo || echo "  (Nodes may still be registering)"

echo ""
echo "Checking cluster status with scontrol:"
podman exec slurm-rshlaunch-head scontrol show nodes || echo "  (Still initializing)"
...
# Run the testsuite
# Based on CI: docker exec slurm-rshlaunch-head bash -c 'cd Spindle-build/testsuite && salloc -n${workers} -N${workers} ./runTests ${workers}'
if podman exec slurm-rshlaunch-head bash -c "cd Spindle-build/testsuite && salloc -n${WORKERS} -N${WORKERS} ./runTests ${WORKERS}"; then
    echo ""
    echo "=========================================="
    ...
    echo "  podman exec -it slurm-rshlaunch-head bash"
    echo "  sinfo"
    echo "  scontrol show nodes"
    echo "  cd Spindle-build/testsuite"
    echo ""
    echo "Manual cleanup when done:"
    echo "  podman rm -f slurm-rshlaunch-mariadb slurm-rshlaunch-db slurm-rshlaunch-head slurm-rshlaunch-node-{1..4}"
    echo "  podman network rm -f slurm-rshlaunch-test-net"
    echo ""
    exit 1
fi
