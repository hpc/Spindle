...
{  
   ...
   buffer_size += args->candidate_locations ? strlen(args->candidate_locations) + 1 : 1;
   buffer_size += args->candidate_cachepaths ? strlen(args->candidate_cachepaths) + 1 : 1;
   buffer_size += args->candidate_commpaths ? strlen(args->candidate_commpaths) + 1 : 1;
   ...
   pack_param(args->candidate_locations, buf, pos);
   pack_param(args->candidate_cachepaths, buf, pos);
   pack_param(args->candidate_commpaths, buf, pos);
...
int getApplicationArgsFE(spindle_args_t *params, int *spindle_argc, char ***spindle_argv)
{
   ...
   *spindle_argv = (char **) malloc(sizeof(char*) * MAX_ARGS);
   ...
   (*spindle_argv)[n++] = strdup(params->candidate_locations);
   (*spindle_argv)[n++] = strdup(params->candidate_cachepaths); /* QQQ FIXME remove when app gets cachepath from server */
   (*spindle_argv)[n++] = strdup(params->candidate_commpaths);
   (*spindle_argv)[n++] = strdup(number_s);
...
int spindleInitFE(const char **hosts, spindle_args_t *params)
{
   ...
   debug_printf("spindle_args_t { number = %lu; port = %u; num_ports = %u; opts = %lu; unique_id = %lu; "
                "use_launcher = %u; startup_type = %u; shm_cache_size = %u; locations = %s; "
                ...
                params->use_launcher, params->startup_type, params->shm_cache_size, params->candidate_locations,
                params->candidate_cachepaths, params->candidate_commpaths,
