...
#define TOTALVIEW_CMD "/usr/global/tools/totalview/m/cab/dflt/bin/totalview"
...
#define DEBUGGER "gdb"
    ...
    printf("SERVERFE: start of cobo_server_open at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);

    /* open the server */
    cobo_server_open(2384932, hostlist, num_hosts, portlist, num_ports);

    printf("SERVERFE: end   of cobo_server_open at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);

    /* wait until tree is ready */
    { 
      int root_fd, ack;
      cobo_server_get_root_socket(&root_fd);
      
      ldcs_cobo_read_fd(root_fd, &ack, sizeof(ack));
      printf("SERVERFE: got  ack=%d from tree root at %10.6f (%6.3f s from start)\n", 
	     ack,__get_time(),__get_time()-starttime);
    }
    ...
    { 
      ...
      printf("SERVERFE: sent ack=%d from tree root at %10.6f (%6.3f s from start)\n", 
	     ack,__get_time(),__get_time()-starttime);
    }

    printf("SERVERFE: start of cobo_server_close at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);

    /* open and close the server */
    cobo_server_close();

    printf("SERVERFE: end of cobo_server_close at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);
    ...
    printf("SERVERFE: exit at %10.6f (%6.3f s from start)\n\n", 
	   __get_time(),__get_time()-starttime);
