...
#define PRELOADFILE_LEN 256
    int preloadfile_on = 0;
    char preloadfile[PRELOADFILE_LEN + 1];
    char *param_env;
    /* FILE *hf; */
		...
		debug_on = 1;
 		...
 		if(tv_env != NULL) {
  		...
  		break;
            case 11:
                preloadfile_on = 1;
                strncpy(preloadfile, optarg, PRELOADFILE_LEN);
                if (strlen(optarg) >= PRELOADFILE_LEN - 1) {
                    preloadfile[PRELOADFILE_LEN] = '\0';
                }
                break;
            case 12:
                usage();
...
#define DEBUGGER "gdb"
    ...
    printf("SERVERFE: start of audit_server_open at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);
    steptime=__get_time();
    ldcs_audit_server_fe_md_open( hostlist, num_hosts, &md_data_ptr );
    ...
    if ( preloadfile_on ) {
      ...
      printf("SERVERFE: found no preload file\n");
    }
    ...
    printf("SERVERFE: start of audit_server_close at %10.6f (%6.3f s from start)\n", 
	   __get_time(),__get_time()-starttime);
