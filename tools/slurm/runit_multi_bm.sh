...
TASKPERCORE="--ntasks-per-core 2 --overcommit"
TASKPERCORE=""

# very specil at LLNL system: sometimes srun mps /tmp to /tmp/frings1
SRUNTMP=/tmp
RSHTMP=/tmp
DEBUGDIR=/p/lscratchb/frings1/log56_32f
#DEBUGDIR=`pwd`/tmp/
...
do
	...
	CMD="SLURM_HOSTFILE=/g/g92/frings1/LLNL/pynamic/benchmark4/tmp/slurm_hostlist_${SPEC}.dat srun --distribution arbitrary -v -n ${TOTALTASKS} ${TASKPERCORE} ${STRACE} pynamic-pyMPI pynamic_driver.py \`date +%s.%N\`"
	echo $CMD > ${STARTFILE}
	chmod u+x ${STARTFILE}
	(
	    echo $CMD
